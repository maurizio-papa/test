...
def make_optimizer(model, optimizer_config):
    ...
    blacklist_weight_modules = (LayerNorm, torch.nn.GroupNorm, torch.nn.BatchNorm2d, torch.nn.AvgPool2d, torch.nn.LayerNorm,)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            if p.requires_grad:
                if fpn in decay or fpn in no_decay:
                ...
                if fpn.endswith('bias'):
                    ...
                    no_decay.add(fpn)
                elif pn.endswith('cls_token') or pn.endswith('pos_embed') or pn.endswith('temporal_embed'):
                    no_decay.add(fpn)
                else:
                    decay.add(fpn)
            else:
